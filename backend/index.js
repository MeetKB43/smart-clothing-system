...
var accuweather = require("node-accuweather")()(
  "lAn0QeTwG0tTqST3rdyTOOYxxO1zGPFQ"
);
...
app.post("/register_device", function (req, res) {
  ...
  // deviceID = 111; uncomment if it dosent work
...
app.post("/dashboard", async function (req, res) {
  ...
  for (i = 0; i < result.length; i++) {
    ...
    var sql3 =
      "SELECT * from inventory WHERE uID = ? AND used = 0 AND cType = 1";
    var sql4 =
      "SELECT * from inventory WHERE uID = ? AND used = 0 AND cType = 2";
    var sql5 =
      "SELECT * from inventory WHERE uID = ? AND used = 0 AND cType = 4";
    var sql6 =
      "SELECT * from inventory WHERE uID = ? AND used = 0 AND cType = 3 AND cSubType = 4";
    const query = util.promisify(con.query).bind(con);
    try {
  ...
  var w = new weather(
    "xtqujVmP2MLpMbK8FT46wAwV5E4HIpzr",
    "Windsor",
    42.314938,
    -83.036362
  );
