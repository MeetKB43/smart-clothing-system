...
export const colorsLight = (_primary = PRIMARY) => {
  const primary = colord(_primary);
  const { h } = primary.toLch();
  ...
  return {
    ...
    shadows: new Array(25).fill(undefined).map((_, i) => {
      ...
      return `0 0 0 1px ${shadowBase.alpha(0.08).toHslString()}, 0 25px 50px -12px ${shadowBase
        .alpha(0.25)
        .toHslString()}, 0 50px 80px ${shadowBase.alpha(0.06).toHslString()}`;
    }),
    ...
    components: {
      ...
      MuiBackdrop: {
        styleOverrides: {
          root: {
            backgroundColor: colord({ l: 70, c: 5, h }).alpha(0.6).toHslString(),
...
export const colorsDark = (_primary = DARK_PRIMARY, darker) => {
  const primary = colord(_primary);
  const { h } = primary.toLch();
  ...
  return {
    ...
    shadows: new Array(25).fill(undefined).map((_, i) => {
      ...
      return `0 0 0 1px ${shadowBase.alpha(0.08 * 4).toHslString()}, 0 25px 50px -12px ${shadowBase
        .alpha(0.25 * 4)
        .toHslString()}, 0 50px 80px ${shadowBase.alpha(0.06 * 4).toHslString()}`;
    }),
